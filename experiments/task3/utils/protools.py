...
def fetch_datasets(
    ...
    if "imagenet" in dataset_name or "vae" in dataset_name:
        ...
        train_dataset = ImageFolder(
            root=f"{ROOT_DIR}/{dataset_name}/train/", transform=train_transforms,
        )
        test_dataset = ImageFolder(
            root=f"{ROOT_DIR}/{dataset_name}/val/", transform=test_transforms,
        )
...
def save_model(
    model, dataset_name: str, model_name: str, model_dir=None, method_name="tuned"
):
    """
    ...
    if model_dir is None:
        ...
        model_file_name = (
            f"{model_dir}{model_name}_{dataset_name}_{method_name}_model.pt"
        )
...
def get_last_checkpoint(
    model_name, checkpoint_dir: os.PathLike = f"../model_files/checkpoints/"
):
...
def save_checkpoint(
    ...
    log.info(
        f"Saved {checkpoint_name}  and train_val_history to '{checkpoint_path}' directory"
    )
...
def load_checkpoint(
    checkpoint_name: str, checkpoint_dir: str = "model_files/checkpoints/"
):
    """
    save a pytorch model to local directory
    save_checkpoint(model, model_name='inception_imagenet-small-subclass_temp_epoch-x'
...
def run_hybrid_dataloader_construction(hybrid_portion: float = 0.5):
    """Demonstration. Basic use.
    ...
    mnist_train_original, mnist_test_original = fetch_datasets(
        original_dataset_name, input_size=image_dim
    )
    mnist_train_original, _ = split_dataset(mnist_train_original, split=0.5, seed=seed)
    mnist_test_original_p1, _ = split_dataset(mnist_test_original, split=0.5, seed=seed)
    ...
    mnist_train_synthetic, _ = split_dataset(
        mnist_train_synthetic, split=0.25, seed=seed
    )
    mnist_test_synthetic_p1, _ = split_dataset(
        mnist_test_synthetic, split=0.5, seed=seed
    )
